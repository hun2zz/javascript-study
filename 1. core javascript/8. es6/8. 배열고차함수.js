...
const mappedUsersAge = userList.map((user) => `${user.age}세`);
console.log(mappedUsersAge);

//회원목록에서 회원 이름과 회원의 나이와 회원의 주소를 추출할ㄹ건데
//이름의 키 값을 nickname으로 바꿔줘 나이는 old, 주소는 addr로 하고 싶음
...
const ageFind = userList.find((user) => user.age >= 50 && user.age < 60);
console.log(ageFind);
