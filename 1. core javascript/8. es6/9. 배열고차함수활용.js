...
/*
사과중에 녹색이면서 당도가 9인 사과만 선별하여 //필터
"이 사과는 xx색이며 당도는 xxx입니다." // 맵 
...
/*
  회원목록에서 서울에 사는 회원들의 두 번째 취미만 추출하여
  `xxx님의 2버째 췸는 xxx입니다`를 반복해서 출력하라.`
*/

userList
  .filter((user) => user.address === `서울`)
  .map((user) => ({ name: user.userName, second: user.hobbys[1] })) // 3명의 취미가 문자열에 들어간다. [`축구`,`테니스`,`테니스`]
  .forEach((result) =>
